...
public:
  ...
  // Only for stereo and RGB-D. This method does not work for monocular.
  // Call first Shutdown()
  void SaveTrajectoryTUMTransformed(const string& filename, const Eigen::Matrix4f& T_target_source,
                                    const double start_timestamp = -1.0,
                                    const std::set<double>& lost_timestamps = {});
  ...
  std::vector<Eigen::Vector3f> GetSparsePointCloud();
