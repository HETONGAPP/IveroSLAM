...
void System::SaveTrajectoryTUMTransformed(const string& filename, const Eigen::Matrix4f& T_target_source,
                                          const double start_timestamp, const std::set<double>& lost_timestamps) {
  Sophus::SE3f T_t_s(T_target_source.block<3, 3>(0, 0), T_target_source.block<3, 1>(0, 3));
  ...
  for (list<Sophus::SE3f>::iterator lit = mpTracker->mlRelativeFramePoses.begin(),
    ...
    // if pose is near a time when we were lost, skip it
    if (!lost_timestamps.empty()) {
      double t = *lT;
      auto lb = lost_timestamps.lower_bound(t);
      if (lb != lost_timestamps.end()) {
        const double closest_timestamp = *lb;
        const auto diff = std::abs(closest_timestamp - t);
    ...
    Sophus::SE3f T_source_world = (*lit) * Trw;
    Sophus::SE3f T_target_world = T_t_s * T_source_world;
    Sophus::SE3f Twc = T_target_world.inverse();

    Eigen::Vector3f twc = Twc.translation();
    Eigen::Quaternionf q = Twc.unit_quaternion();

    f << setprecision(9) << *lT << " " << setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x()
      << " " << q.y() << " " << q.z() << " " << q.w() << endl;
  }
  f.close();
...
void System::SaveTrajectoryEuRoC(const string& filename) {
  ...
  for (Map* pMap : vpMaps) {
    std::cout << "  Map " << std::to_string(pMap->GetId()) << " has " << std::to_string(pMap->GetAllKeyFrames().size())
              << " KFs" << std::endl;
...
void System::SaveKeyFrameTrajectoryEuRoC(const string& filename) {
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD) {
      ...
      Eigen::Quaternionf q = Twc.unit_quaternion();
      Eigen::Vector3f t = Twc.translation();
      f << setprecision(6) << 1e9 * pKF->mTimeStamp << " " << setprecision(9) << t(0) << " " << t(1) << " " << t(2)
        << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
    }
  }
  f.close();
...
std::vector<Eigen::Vector3f> System::GetSparsePointCloud() {
